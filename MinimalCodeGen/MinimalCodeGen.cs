...
public static class Program
{
    public static int Main(string[] args)
    {
        if (args.Length != 2)
        {
            Console.Error.WriteLine("usage: MinimalCodeGen <in.lex> <out.asm>");
            return 1;
        }
        using (var input = new StreamReader(args[0]))
        using (var output = new StreamWriter(args[1], false, new UTF8Encoding(false)))
        {
            var gen = new MinimalCodeGen(output);
            gen.Run(input);
            // coverage report to stderr
            if (gen.NotImplemented.Count > 0)
            {
                Console.Error.WriteLine($"-- {gen.NotImplemented.Count} opcodes not yet implemented:");
                foreach (var kv in gen.NotImplemented)
                    Console.Error.WriteLine($"   {kv.Key}  ({kv.Value}x)");
            }
        }
        return 0;
    }
