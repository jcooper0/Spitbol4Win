...
; ---- runtime interface (defined in int.asm / osint) ------------
;  Like the sysXX entry points, these symbols are supplied by the
;  runtime and resolved at link. The NASM build declares them in its
;  prepended header; the generated body references but never declares
;  them, so the header must. Types follow usage:
;     call target            -> PROC
;     ldmxcsr/stmxcsr [m32]   -> DWORD
;     word-sized data cell    -> QWORD
;  This is the complete set the current body references (verified by
;  assembling to convergence). Add further runtime symbols here as the
;  body comes to use them.
;
;  routines (call targets):
EXTERN sin_:PROC                   ; real math intrinsics
EXTERN cos_:PROC
EXTERN tan_:PROC
EXTERN atn_:PROC
EXTERN sqr_:PROC
...
EXTERN chp_:PROC
;  data cells:
EXTERN mxcsr_set:DWORD             ; ldmxcsr preset rounding/mask word
EXTERN _rc_:QWORD                  ; return-code cell
EXTERN zeron:QWORD                 ; real constant 0.0
EXTERN lowspminx:QWORD             ; low stack-pointer limit
