#ifndef OSINT_PORT_H_INCLUDED
#define OSINT_PORT_H_INCLUDED
...
*/
#include "systype.h"
#include "oswin.h"   /* Windows compat (no-op on Linux) */
...
#ifndef EXTFUN
#define EXTFUN          1       /* external functions enabled (Windows: LoadLibrary/GetProcAddress via syslinux.c) */
#endif
...
#ifdef _WIN32
typedef long long word;            /* LLP64: Windows 'long' is 32-bit; word must be 64-bit */
...
/*  MINIMAL word types for the external-function layer (sproto.h, sysld.c,
 *  callef.c, syslinux.c).  Under PRIVATEBLOCKS these come from extern32.h as
 *  32-bit int/unsigned int; on the spitblks path they were undefined, which
 *  broke sproto.h's "typedef mword (*PFN)();" in every translation unit.  They
 *  must be word-width: a loaded DLL handle and function-pointer return are
 *  64-bit on Win64 and would be truncated by a 32-bit mword. */
#ifndef PRIVATEBLOCKS
typedef word  mword;         /* MINIMAL word (word-width, not int) */
typedef uword muword;        /* MINIMAL unsigned word */
#endif
